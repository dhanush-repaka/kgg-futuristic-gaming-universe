...
export default function KGGOfferings() {
  return (
    <section id="games" className="mx-auto w-full max-w-7xl px-5 py-20 md:px-8">
      <Reveal>
      ...
      <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3" style={{ perspective: "1000px" }}>
        {offerings.map((item, index) => {
          const Icon = item.icon;
          return (
            <Reveal key={item.title} delay={index * 0.08}>
              <motion.article
                whileHover={{ rotateX: -4, rotateY: index % 2 === 0 ? 6 : -6, y: -8 }}
                transition={{ type: "spring", stiffness: 220, damping: 20 }}
                className="group relative h-full rounded-2xl border border-white/12 bg-white/5 p-6 backdrop-blur-xl"
              >
                <div className="pointer-events-none absolute inset-0 rounded-2xl bg-[radial-gradient(circle_at_top_right,rgba(34,211,238,0.14),transparent_40%)] opacity-0 transition duration-500 group-hover:opacity-100" />
