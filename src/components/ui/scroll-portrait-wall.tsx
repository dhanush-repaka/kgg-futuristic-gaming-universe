...
export interface ScrollPortraitWallProps {
  ...
  /** Keep portraits in color (game covers). Default applies grayscale. */
  colorful?: boolean;
  className?: string;
}
                ...
                <div key={ci} className="aspect-square flex-1">
                  ...
                  >
                    ...
                    <img
                      ...
                      draggable={false}
                      className={cn(
                        "h-full w-full object-cover transition-transform duration-500 ease-in-out hover:scale-95",
                        colorful ? "" : "grayscale contrast-[1.15] filter",
                      )}
                    />
                    {showCaptions && (
                      <div className="absolute -bottom-2 left-0 flex w-full translate-y-full justify-between gap-2 text-[11px] uppercase leading-tight text-muted-foreground sm:text-sm">
                        ...
                        <span
                          className={cn(
                            "shrink-0",
                            s.role === "Coming Soon" && "font-semibold text-accent",
                          )}
                        >
                          ({s.role})
                        </span>
